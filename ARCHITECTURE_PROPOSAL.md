...
JDTLS is enabled through `plugins.java`; `plugins.jdtls` is not enabled. The global `cmp_nvim_lsp` capability path is reused, so completion does not create a second capability configuration. The Java configuration imports Maven and Gradle projects automatically during the first startup and after build-file changes; an interactive import prompt can leave JDTLS with an incomplete classpath and make `nvim-cmp` expose only buffer text. Structural JDTLS options (`cmd`, `cmd_env`, `init_options.bundles`, `root_markers` and filetypes) remain owned by `nvim-java`; the local post-configuration contributes capabilities and Java settings only, avoiding index-wise list collisions between two `vim.lsp.config` calls. When a completion-capable client attaches after `InsertEnter`, the completion layer replays that supported event for the affected buffer so `cmp-nvim-lsp` can register its source without mutating private plugin state; the detach path performs the same synchronization so stopped clients are removed by the upstream owner. Completion also enables automatic imports, organize-imports on save, project-aware source roots, Maven/Gradle markers, code lenses, formatting, diagnostics and multiple JDK runtimes.

The Java toolchain is externalized to Nix paths. The consuming `nix-conf` lock currently supplies JDTLS 1.58.0, JDK 8/21/25, Lombok 1.18.42, Java Test 0.45.0 and Java Debug 0.59.0; the standalone `vim-conf` lock may evaluate a different nixpkgs revision and must not be treated as the consuming host's package set. nvim-java uses the supplied paths with `auto_install = false`, so startup never downloads a mutable tool. Spring Tools auto-install is disabled because the matching VS Code extension is not provided as a native package in the pinned nixpkgs set; the Spring Boot plugin remains available without introducing another Java LSP owner. JDTLS keeps one hashed workspace per project under the Neovim cache, so its project model and incremental state are not rebuilt on every buffer.

Java formatting is owned by JDTLS. Conform does not issue a competing Java LSP format request, while the explicit organize-import action remains available as a fallback. Diagnostics are presented through `vim.diagnostic`, and Neotest/DAP use the nvim-java-provided Java integrations. Fidget remains the single progress renderer; Noice does not duplicate LSP progress, and only the repetitive JDTLS tasks `Validate documents` and `Publish Diagnostics` are filtered from Fidget while LSP messages and diagnostic data remain available.
...
[10]: https://neovim.io/doc/user/lsp/ "Neovim LSP documentation"
[11]: https://github.com/hrsh7th/nvim-cmp/blob/main/doc/cmp.txt "nvim-cmp documentation"
[12]: https://github.com/hrsh7th/cmp-nvim-lsp/blob/main/lua/cmp_nvim_lsp/init.lua "cmp-nvim-lsp source registration"
[13]: https://neovim.io/doc/user/api.html "Neovim API documentation"
[14]: https://nix.dev/concepts/flakes.html "Nix flakes documentation"
