...
Java formatting is owned by JDTLS. Conform does not issue a competing Java LSP format request, while the explicit organize-import action remains available as a fallback. Diagnostics are presented through `vim.diagnostic`, and Neotest/DAP use the nvim-java-provided Java integrations. Fidget remains the single progress renderer; Noice does not duplicate LSP progress, and only the repetitive JDTLS tasks `validate documents` and `published diagnostics` are filtered from Fidget while LSP messages and diagnostic data remain available.
