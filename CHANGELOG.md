...
## 2026-09-01 — IntelliJ LSP feasibility assessment

The current Java workflow is tightly coupled to the Nix-packaged JDTLS contract: `nvim-jdtls` supplies project workspaces and Java-specific import commands, while `neotest-java`, Lombok agent loading and the existing DAP workflow are already validated around it. The new IntelliJ Java/Kotlin LSP offering was therefore reviewed as an architecture and distribution question rather than treated as a drop-in replacement.
