...
Advanced file creation and the project creator use documented Snacks APIs (`Snacks.input`, `Snacks.picker.pick` and `vim.ui.select`). The creator is hierarchical: it first selects a language, then a framework or project type, then the build system or structure. Entries include Java plain/Spring Boot/JavaFX with Maven or Gradle, JavaScript Vite/React/Vue/Svelte/Next.js, TypeScript Vite/Angular/NestJS/Next.js, Python uv/FastAPI/Django, Go modules/Gin/Chi, PHP Composer/Laravel, C/C++ CMake and Rust Cargo. Names are human-readable without internal icon identifiers, the picker is larger and the preview shows a realistic directory tree with folder and file icons, including expanded package paths such as `src/main/java/com/example/app`. Its title box is generated from the display width of the exact title and closes on all four corners without truncating the horizontal rule. The default destination is `~/Projects`, which is created automatically when absent; another destination must already exist. Generators install or manage dependencies only when the selected tool is available and invoke commands through argv without shell concatenation. Spring Initializr options come from its live metadata endpoint; Nix evaluation remains free of network access.

## Languages and toolchains

The configuration includes LSP support for Lua, Nix, Bash, Docker, C/C++, Python, Rust, Go, Markdown, Angular, TypeScript, ESLint, HTML, CSS, JSON, YAML, Emmet and Tailwind. It also includes PHP, PostgreSQL-aware SQL, XML/XSD/XSLT/SVG, CMake Tools, OpenOCD, Cppcheck, Competitest and a declarative LeetCode workflow under `<leader>p`.

The supported coverage is organized as follows:

| Ecosystem | Support | LSP/completion owner |
|---|---|---|
| Java, Spring Boot, JavaFX | Dedicated local JDTLS workflow with Maven, Gradle, nvim-java, Neotest and DAP | nvim-java/JDTLS + `nvim-cmp` |
...
Java is a dedicated local workflow backed by JDTLS and orchestrated by nvim-java. JDTLS, the JDK, Lombok, Java Test and Java Debug Adapter are provided by the consuming Nix configuration and referenced by fixed store paths; no nvim-java tool downloads occur at startup. The consuming `nix-conf` lock currently resolves JDTLS 1.58.0, while the standalone `vim-conf` lock may resolve another nixpkgs revision, so the consumer lock is the source of truth for the active host. Spring Tools and the standalone Spring Boot plugin are disabled so they cannot create a competing client rooted at the home directory.

Completion uses the shared `nvim-cmp` sources and explicit `cmp_nvim_lsp.default_capabilities()` on the nvim-java JDTLS client. The fixed NixVim revision emits the native `cmp.setup` configuration, including `completion.autocomplete` on `InsertEnter` and `TextChanged`; no parallel timer or manual `cmp.complete()` call is installed. The native nvim-cmp pipeline uses a 60 ms debounce and 30 ms throttle, so each change is coalesced once by the completion engine instead of waiting for an additional 250 ms timer. LSP, snippet, path and buffer sources share one group with explicit priorities so repeated text remains available while semantic candidates appear first; path and buffer use a one-character threshold. The `nvim_lsp` source is synchronized by replaying the official `cmp-nvim-lsp` `InsertEnter` registration after a completion-capable client attaches or detaches, so delayed JDTLS startup does not leave the source unavailable and no private source registry is mutated. JDTLS provides diagnostics, completion, references, code actions and formatting; the current configuration explicitly enables `java.completion.enabled` and disables `java.completion.lazyResolveTextEdit.enabled` so import text edits are returned directly, while `saveActions.organizeImports` remains available as a separate action. Conform does not issue a competing Java LSP format request. Maven/Gradle source and Javadoc attachment stays available through explicit project actions instead of blocking the first JDTLS import. The per-project JDTLS workspace remains cached under Neovim's state directory, project-root detection is restricted to Java build markers and rootless single-file clients are disabled. `<leader>jl` opens a runtime completion report, `<leader>jh` checks LSP health, `<leader>jo` runs an explicit organize-import fallback, `<leader>jr` runs the current main class, `<leader>jR` builds the workspace, and `<leader>jt`/`<leader>jT` run or debug the nearest test.
...
nvim --headless -u NONE -l tests/luasnip_fmt_spec.lua
