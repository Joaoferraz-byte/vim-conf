...
Advanced file creation, the project picker and the project creator use documented Snacks APIs (`Snacks.input`, `Snacks.picker.pick` and `vim.ui.select`). The creator previews a tree for Java/Maven, Java/Gradle, Spring Initializr, Python, Rust, Go, CMake, Zig and TypeScript templates, validates the destination, and invokes the selected tool through argv without shell concatenation. Spring Initializr options come from its live metadata endpoint; Nix evaluation remains free of network access.
